...
**Your focus here**
- Validate consistency across `backend/analysis/**`, `risk_manager.py`, `paper_broker.py`.
- Concrete suspected dimensional bugs to confirm: `min_confidence` percent compared to a 0–1
  fraction; volatility annualization `sqrt(252)` applied to intraday bars
  (`analysis/volatility/indicators.py`); SL/target expressed in points but compared to ₹ prices.
