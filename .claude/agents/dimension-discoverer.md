...
**Your focus here**
- Derive the dimensional vocabulary from `backend/analysis/`, `backend/utils/risk_manager.py`,
  `backend/services/paper_broker.py`, and the option-chain handling in `market_routes.py`.
- Expect: index points, ₹ price, percent (note inconsistent 0–100 vs 0–1), lots vs quantity
  (lot size matters for NIFTY/BANKNIFTY), option premium, P&L, bps, and a 0–1 confidence.
