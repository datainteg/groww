...
**Your focus here**
- Propagate dimensions through the signal pipeline: candle OHLCV → indicators
  (`analysis/momentum`, `volatility`, `support_resistance`) → `decision_engine` /
  `market_direction_engine` confidence → `risk_manager` SL/target → `paper_broker` P&L.
- Flag the confidence fraction↔percent crossing (`trading_engine.py:162` vs
  `decision_engine.py:128`) and any points↔₹ mixing in SL/target/P&L.
