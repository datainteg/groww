...
**Your focus here**
- Own the broker/order layer: `services/groww_client.py` (auth token, LTP/quote/OHLC,
  order CRUD, margins, positions), `services/paper_broker.py` (fill simulation), and order
  routes in `trade_routes.py`.
- Make paper fills realistic: model **slippage, brokerage/STT/exchange fees, and latency**
  for options — currently fills are near-frictionless, which makes results optimistic.
- LIVE-mode hardening: idempotency keys per logical order; fail-**closed** trade lock;
  real `get_margins` balance (the `/trade/limits` LIVE branch is a `0` stub); implement true
  position reconciliation (the scheduler's `reconcile_positions` is a no-op).
- Track Groww token expiry/refresh; a 401 currently freezes the feed silently.
