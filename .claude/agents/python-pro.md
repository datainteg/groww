...
**Your focus here**
- Codebase is Python 3.10, Flask, pymongo, redis-py, pandas/numpy, APScheduler, `requests`
  (currently **synchronous** everywhere). Match existing style; add type hints where missing.
- High-value modernization: shared `requests.Session` + retry/backoff (replace the duplicate
  raw `requests.get` in `candle_service` with the unified `GrowwClient`); thread-safe
  singletons; avoid per-tick object construction in the 5s heartbeat.
- Don't introduce asyncio into the Flask sync stack without a clear migration plan.
