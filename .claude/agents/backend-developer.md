...
**Your focus here**
- Own `backend/app.py`, `routes/`, `services/`, `database/`. Respect the singleton pattern
  for `mongodb` and `redis_client`.
- High-priority known defects to fix or avoid regressing: scheduler `__main__`-only start;
  no HTTP retry/backoff in `groww_client._make_request` (`groww_client.py:85-166`);
  fail-open Redis trade lock (`trade_routes.py:22-45`); candle sync uses destructive
  `delete_many`+`insert_many` every 60s (`candle_service.py:212-220`) — prefer the upsert path.
- `GrowwClient` is re-instantiated every heartbeat (DB hit + decrypt every 5s) — cache it.
- Keep everything IST-aware and closed-candle-correct.
