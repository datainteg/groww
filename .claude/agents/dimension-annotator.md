...
**Your focus here**
- Annotate `backend/analysis/**`, `backend/utils/risk_manager.py`, and
  `backend/services/paper_broker.py`.
- Project dimension vocabulary: `Price{₹}`, `IndexPoints`, `Percent{0-100}` vs `Frac{0-1}`,
  `Lots`, `Qty` (= Lots × lot_size), `Premium{₹}`, `PnL{₹}`, `Bps`, `Confidence{0-1}`.
  The confidence percent-vs-fraction and SL/target points-vs-₹ boundaries are the highest-risk
  conversion sites — annotate them explicitly.
